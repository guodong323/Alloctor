...
/*
 certn_size_mem_page * bin_serach(certn_size_mem_page **page_ary, certn_size_mem_page *page, size_t l, size_t r) {
     assert(*page_ary <= page);
     for (; l <= r; --r) {
         if (page_ary[r] < page) {
             return page_ary[r];
         }
     ...
     while (l < r) {
         ...
         if (page_ary[m] > page) {
             r = m - 1;
         }else if (page_ary[m] < page) {
             if (page_ary[m+1] > page) {
                 return page_ary[m];
             }else {
                 l = m + 1;
             }
